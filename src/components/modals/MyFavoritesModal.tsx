...
export default function MyFavoritesModal({ onClose, savedData = [] }: Props) {
  ...
  const [groupedFavourites, setGroupedFavourites] = useState<Record<string, Record<string, FavouriteField[]>>>({})
  ...
  // Get the latest saved data for a specific field
  const getFieldValue = (reportType: string, fieldName: string): string => {
    if (!savedData || savedData.length === 0) return ""
    
    // Get the most recent test data
    const latestTest = [...savedData]
      .filter(test => {
        const data = test[reportType as keyof PatientTest]
        return data && typeof data === 'object' && fieldName in (data as any)
      })
      .sort((a, b) => {
    ...
    const data = latestTest[reportType as keyof PatientTest] as any
    ...
    if (typeof fieldData === 'string') {
      ...
      // For fields with value/notes or value1/value2 structure
      if ('value' in fieldData && typeof fieldData.value === 'string') {
        return fieldData.value
      } else if ('value1' in fieldData && typeof fieldData.value1 === 'string') {
        return fieldData.value1
      } else if ('notes' in fieldData && typeof fieldData.notes === 'string') {
        return fieldData.notes
      }
    }
  ...
  const loadFavourites = () => {
    ...
    // Group by report type, then by section
    ...
    favs.forEach(fav => {
      ...
      const sectionKey = (fav.sectionTitle && fav.sectionTitle.trim() !== '') ? fav.sectionTitle : 'Other'
  ...
  const handleValueChange = (reportType: string, fieldName: string, value: string) => {
  ...
  const handleRemoveSection = (reportType: string, sectionTitle: string, fields: FavouriteField[]) => {
    ...
    fields.forEach(fav => {
      ...
      if (fav.reportType === 'autoimmunoProfile' && !fav.fieldName.endsWith('_notes')) {
      ...
      if ((fav.reportType === 'rft' || fav.reportType === 'lft') && fav.fieldName.endsWith('_value1')) {
      ...
      if ((fav.reportType === 'rft' || fav.reportType === 'lft') && fav.fieldName.endsWith('_value2')) {
          ...
          {favourites.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Click the heart icon on any field in the test modals to add it to favorites
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              {Object.entries(groupedFavourites).map(([reportType, sections]) => {
                ...
                return (
                  <div key={reportType} className="bg-gray-50 rounded-lg p-4 border">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">
                      {reportTypeLabels[reportType] || reportType}
                    </h3>
                    <div className="space-y-6">
                      {Object.entries(sections).map(([sectionTitle, fields], sectionIndex) => {
                        ...
                        return (
                          <div key={`${reportType}-${sectionTitle}`} className="space-y-3">
                            {/* Always show section header with delete button */}
                            <div className="flex items-center justify-between mb-3 border-b pb-2">
                              <h4 className="font-semibold text-base text-blue-700">
                                {sectionTitle === 'Other' ? 'Other Fields' : sectionTitle}
                              </h4>
                              <button
                                onClick={() => handleRemoveSection(reportType, sectionTitle, fields)}
                            ...
                            <div className="space-y-2">
                              {fields.map((fav, fieldIndex) => {
                                ...
                                const colorIndex = (sectionIndex * fields.length + fieldIndex) % fieldColors.length
                                const colorClass = fieldColors[colorIndex]
                                
                                // Check if this is a notes field or value2 field
                                const isNotesField = fav.fieldName.endsWith('_notes')
                                const isValue2Field = fav.fieldName.endsWith('_value2')
                                const isValue1Field = fav.fieldName.endsWith('_value1')
                                
                                // Skip value2 fields - they will be shown with value1 fields
                                if (isValue2Field) {
                                  const mainFieldName = fav.fieldName.replace('_value2', '')
                                  const value1FieldExists = fields.find(f => f.fieldName === `${mainFieldName}_value1`)
                                  if (value1FieldExists) {
                                    return null // Skip value2 field, will be shown with value1 field
                                  }
                                }
                                
                                const mainFieldName = isNotesField ? fav.fieldName.replace('_notes', '') 
                                  : isValue1Field ? fav.fieldName.replace('_value1', '')
                                  : isValue2Field ? fav.fieldName.replace('_value2', '')
                                  : fav.fieldName
                                
                                const notesKey = isNotesField ? key : `${fav.reportType}:${mainFieldName}_notes`
                                const savedNotes = fieldValues[notesKey] || ''
                                
                                // For dual value fields (RFT, LFT, etc.), get value2
                                const value2Key = isValue1Field ? `${fav.reportType}:${mainFieldName}_value2` : null
                                const savedValue2 = value2Key ? fieldValues[value2Key] || '' : ''
                                ...
                                if (fav.reportType === 'autoimmunoProfile' && isNotesField) {
                                  // Skip notes field here, it will be shown with the main field
                                  const mainFieldExists = fields.find(f => f.fieldName === mainFieldName)
                                  if (mainFieldExists) {
                                      ...
                                      <div className="flex-1 min-w-0 space-y-3">
                                        <div>
                                          <p className="font-medium text-gray-900 text-sm">
                                            {(() => {
                                              if (isNotesField) {
                                                return fav.fieldLabel.replace(' - Notes', '')
                                              } else if (isValue1Field || isValue2Field) {
                                                // For value1/value2 fields, extract main field name
                                                return fav.fieldLabel.split(' - ')[0]
                                              }
                                              return fav.fieldLabel
                                            })()}
                                          </p>
                                          <p className="text-xs text-gray-500 mt-1">{fav.fieldName}</p>
                                        </div>
                                        
                                        {/* For AutoimmunoProfile, show both Value (Select) and Notes */}
                                        {fav.reportType === 'autoimmunoProfile' && !isNotesField && !isValue1Field && !isValue2Field ? (
                                          <div className="space-y-3">
                                            {/* Value field (Select) */}
                                            <div className="space-y-2">
                                              <Label htmlFor={`value-${key}`} className="text-xs font-medium text-gray-700">
                                                Value (Select)
                                              </Label>
                                              <Input
                                                id={`value-${key}`}
                                                type="text"
                                                value={savedValue}
                                                onChange={(e) => handleValueChange(fav.reportType, fav.fieldName, e.target.value)}
                                            ...
                                            {/* Notes field */}
                                            <div className="space-y-2">
                                              <Label htmlFor={`notes-${notesKey}`} className="text-xs font-medium text-gray-700">
                                                Notes
                                              </Label>
                                              <Input
                                                id={`notes-${notesKey}`}
                                                type="text"
                                                value={savedNotes}
                                                onChange={(e) => handleValueChange(fav.reportType, `${mainFieldName}_notes`, e.target.value)}
                                                placeholder="Enter notes..."
                                                className="bg-white text-sm"
                                              />
                                            </div>
                                          </div>
                                        ) : isValue1Field && savedValue2 !== undefined ? (
                                          /* For dual value fields (RFT, LFT, etc.), show both value1 and value2 */
                                          <div className="space-y-3">
                                            <div className="grid grid-cols-2 gap-3">
                                              {/* Value 1 */}
                                              <div className="space-y-2">
                                                <Label htmlFor={`value1-${key}`} className="text-xs font-medium text-gray-700">
                                                  {getUnitFromLabel(fav.fieldLabel) || 'Value (Unit 1)'}
                                                </Label>
                                                <Input
                                                  id={`value1-${key}`}
                                                  type="text"
                                              ...
                                              {/* Value 2 */}
                                              <div className="space-y-2">
                                                <Label htmlFor={`value2-${value2Key}`} className="text-xs font-medium text-gray-700">
                                                  {(() => {
                                                    const value2Field = fields.find(f => f.fieldName === `${mainFieldName}_value2`)
                                                    return value2Field ? getUnitFromLabel(value2Field.fieldLabel) || 'Value (Unit 2)' : 'Value (Unit 2)'
                                                  })()}
                                                </Label>
                                                <Input
                                                  id={`value2-${value2Key}`}
                                                  type="text"
                                                  value={savedValue2}
                                                  onChange={(e) => handleValueChange(fav.reportType, `${mainFieldName}_value2`, e.target.value)}
                                                  placeholder="Enter value..."
                                                  className="bg-white text-sm"
                                                />
                                              </div>
                                          ...
                                          <div className="space-y-2">
                                            ...
                                            <Input
                                              ...
                                              value={savedValue}
                                              onChange={(e) => handleValueChange(fav.reportType, fav.fieldName, e.target.value)}
                                              placeholder={isNotesField ? "Enter notes..." : "Enter value..."}
                                              className="bg-white text-sm"
                                            />
                                          </div>
                                        )}
